...
case class Disagreement[PropType](
  locutor: Interlocutor,
  content: PropType) extends StandardDialogueAct[PropType]
  with TaskDialogueAct {
  val name: String = Disagreement.name

}

/**
 * Factory for [[fr.dubuissonduplessis.dogma.ditpp.informationTransfer.Disagreement]] instances and descriptions of such events.
...
object Disagreement {
   ...
   * @tparam PropType type of the content of the "disagreement" dialogue act
   * @return a description of this "disagreement" dialogue act with an uninstantiated semantic content
   */
  def apply[PropType](
    interlocutor: Interlocutor,
    variable: Variable[PropType]): EventDescription =
    new DialogueActEventDescription[PropType, EventDescription] {
      def name: String = Disagreement.name

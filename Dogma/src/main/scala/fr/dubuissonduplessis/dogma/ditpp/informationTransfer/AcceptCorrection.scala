...
case class AcceptCorrection[Content1, Content2](
  locutor: Interlocutor,
  content1: Content1,
  content2: Content2)
  extends NonStandardDialogueAct[Content1, Content2]
  with TaskDialogueAct {
  val name: String = AcceptCorrection.name
}

/**
 * Factory for [[fr.dubuissonduplessis.dogma.ditpp.informationTransfer.AcceptCorrection]] instances and descriptions of such events.
 * @author Guillaume DUBUISSON DUPLESSIS <guillaume.dubuisson_duplessis@insa-rouen.fr>
 */
object AcceptCorrection {
  /**
   * Name of this dialogue act.
   */
  val name: String = "acceptCorrection"
   ...
   * @tparam PropType type of the content of the "acceptCorrection" dialogue act
   * @return a description of this "acceptCorrection" dialogue act with an uninstantiated semantic content
   */
  def apply[PropType](
    ...
    GenericNonStandardDialogueActEventDescription(
      AcceptCorrection.name,
      interlocutor,
      variable1, variable2,
      (i: Interlocutor, c1: PropType, c2: PropType) => AcceptCorrection(i, c1, c2))
}
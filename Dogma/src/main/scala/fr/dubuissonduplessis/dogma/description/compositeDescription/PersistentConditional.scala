...
/**
 * Represents a persistent conditional description.
 * Persistent conditional descriptions take the following form : (desc1|desc2|...|descn) =*=> op where each "desci"
 * is a description of a single event, and "op" is an [[fr.dubuissonduplessis.dogma.operation.Operation]].
 * They differ from [[fr.dubuissonduplessis.dogma.description.compositeDescription.Conditional]] in that they are persistent.
 *
 * For instance:
 * {{{
 ...
 * }
 *
 * // Persistent Conditional description
 * val conditional = alt =*=> op
 * //> conditional  : fr.dubuissonduplessis.dogma.description.compositeDescription.PersistentConditional =
 ...
 *  <td style="border-width: 1px; padding: 3px; border: 1px solid black; text-align: left">
 *  	<strong>Delegated to desc and op</strong> <br/>
 *   	Note that a persistent conditional is instantiated as a Conditional to ensure persistence.<br/>
 *    	The created Conditional will be consumed by its application while the persistence is left "as is", and persists.<br/>
 *   	Equivalent to: desc.instantiateWith(s) <strong>==></strong> op.instantiateWith(s)
 *  </td>
 * </tr>
 * <tr>
 ...
 * @constructor Creates a new persistent conditional description.
 * @param premisse a single event description, or an alternative description
 * @param op an operation
 */
case class PersistentConditional(premisse: EventsDescription, op: Operation)
  extends Description
  ...
  protected def update(instantiable1: EventsDescription, instantiable2: Operation): Conditional =
    // A PersistentConditional is instantiated as a Conditional
